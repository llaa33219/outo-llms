...
This guide uses the default local server address, llama.cpp, and a small GGUF model from Hugging Face. It assumes Python 3.10 or newer and a working `pipx` or `pip` installation.
...
This command accepts all defaults without prompting, while still announcing and logging each action. It installs llama.cpp into its own engine virtual environment, keeps the API on local HTTP, and does not change the firewall.
...
Signup is open and creates a user, a `default` workspace, and the first API key. The key is returned in plaintext only in this response, so copy it immediately.
...
```bash
outo-llms setup --engine vllm --no-https --no-open-port --yes
outo-llms models add tinyllama \
...
Use the API key as a Bearer token. The proxy resolves `tinyllama` in the registry, starts the active engine on an internal loopback port if necessary, forwards the request, and returns the upstream OpenAI-style response.
...
The engine starts on demand. llama.cpp listens internally on port `8612` by default. The client should continue to use port `8611`, the managed API server port.
